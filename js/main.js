...
function init() {
    ...
    colorList.push(['#FCEEDD', '#FFF8EF', '#FFF8EF']);//重ねる波の色設定
  // 各キャンバスの初期化
...
function draw(canvas, color) {
    ...
    drawWave(canvas, color[0], 1, 0.8, 210);//0.5⇒透過具合50%、3⇒数字が大きいほど波がなだらか
    drawWave(canvas, color[0], 0.6, 1.2, 250);
    drawWave(canvas, color[0], 0.9, 1, 160);
}
...
function drawWave(canvas, color, alpha, zoom, delay) {
    ...
    context.lineTo(canvas.width + 120, canvas.height); //パスをCanvasの右下へ
